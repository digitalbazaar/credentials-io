/* global module require */
/* jshint -W097 */
'use strict';

var mockData = require('./mock.data');

// jscs:disable
var didDocument = {
  "@context": "https://w3id.org/identity/v1",
  "id": "did:32e89321-a5f1-48ff-8ec8-a4112be1215c",
  "idp": "did:bef5ac6a-ca9c-4548-8179-76b44692bb86",
  "accessControl": {
    "writePermission": [
      {
        "id": "did:32e89321-a5f1-48ff-8ec8-a4112be1215c/keys/1",
        "type": "CryptographicKey"
      },
      {
        "id": "did:d1d1d1d1-d1d1-d1d1-d1d1-d1d1d1d1d1d1",
        "type": "Identity"
      }
    ]
  },
  "publicKey": [
    {
      "id": "did:32e89321-a5f1-48ff-8ec8-a4112be1215c/keys/1",
      "type": "CryptographicKey",
      "owner": "did:32e89321-a5f1-48ff-8ec8-a4112be1215c",
      "publicKeyPem": "-----BEGIN PUBLIC KEY-----\r\\r\nNwIDAQAB\r\n-----END PUBLIC KEY-----\r\n"
    }
  ],
  "signature": {
    "type": "GraphSignature2012",
    "created": "2015-07-02T21:45:26Z",
    "creator": "did:32e89321-a5f1-48ff-8ec8-a4112be1215c/keys/1",
    "signatureValue": ""
  }
};

// jscs:enable
var ownerDocs = {};
ownerDocs.mockIssuer = {
  '@context': 'https://w3id.org/identity/v1',
  publicKey: mockData.keys.identitySigning.publicKeyUrl
};
ownerDocs.mockAuthority = {
  '@context': 'https://w3id.org/identity/v1',
  publicKey: mockData.keys.credentialSigning.publicKeyUrl
};

var keys = {};
keys.publicKeys = {};
keys.publicKeys.mockIssuer = {
  '@context': 'https://w3id.org/identity/v1',
  id: mockData.keys.identitySigning.publicKeyUrl,
  type: 'CryptographicKey',
  owner: mockData.keys.identitySigning.owner,
  publicKeyPem: mockData.keys.identitySigning.publicKeyPem
};
keys.publicKeys.mockAuthority = {
  '@context': 'https://w3id.org/identity/v1',
  id: mockData.keys.credentialSigning.publicKeyUrl,
  type: 'CryptographicKey',
  owner: mockData.keys.credentialSigning.owner,
  publicKeyPem: mockData.keys.credentialSigning.publicKeyPem
};

module.exports = {
  didDocument: didDocument,
  keys: keys,
  ownerDocs: ownerDocs
};
